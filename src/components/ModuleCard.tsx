      ...
      <div
        ...
        style={
          isActive && highlighted
            ? { boxShadow: "0 0 12px hsl(183 100% 50% / 0.6)" }
            : undefined
        }
      />
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 mb-3">
          <div className="flex items-center gap-3">
            <span className="font-mono text-xs cyan-text tracking-wider">{num}</span>
            <span className="font-mono text-xs text-muted-foreground">/</span>
            <h3 className="font-mono text-sm md:text-base font-medium text-foreground">{title}</h3>
          </div>

          {highlighted && (
            <div className="flex items-center gap-2 px-3 py-1 rounded-sm border border-primary/30 bg-primary/5 pulse-cyan shrink-0">
